...
const views = {
  overview: {
    title: "Обзор",
    subtitle: "Состояние сетей, узлов и клиентов",
  ...
  nodes: {
    title: "Узлы",
    subtitle: "Клиенты и маршруты внутри выбранной сети",
  ...
  els.networksTable.innerHTML = state.networks
    .map(
      (network) => `<tr>
        ...
        <td>${(network.dns || []).map((dns) => `<span class="pill">${escapeHTML(dns)}</span>`).join("") || "none"}</td>
...
function renderNodes() {
  ...
  if (!state.selectedNetwork) {
    els.nodesTable.innerHTML = `<tr><td colspan="5">Выберите сеть</td></tr>`;
    return;
  }
  if (nodes.length === 0) {
    els.nodesTable.innerHTML = `<tr><td colspan="5">Узлов нет</td></tr>`;
    return;
  }
  els.nodesTable.innerHTML = nodes
    .map(
      (node) => `<tr>
        ...
        <td class="mono">${escapeHTML(node.endpoint || "none")}</td>
        <td>${(node.tags || []).map((tag) => `<span class="pill">${escapeHTML(tag)}</span>`).join("") || "none"}</td>
...
function renderTopology() {
  ...
  if (nodes.length === 0) {
    els.topology.innerHTML = `<div class="empty-state">${escapeHTML(network.name)}: узлов нет</div>`;
    return;
  }
  const width = 900;
  const height = 420;
...
function renderCommands() {
  ...
  const installURL = new URL("install.sh", window.location.href).href;
